...
public class KiwiAI : MonoBehaviour
{
    [Header("References")]
    public Transform player;                 // auto-find by tag if empty
    public Animator animator;                // Animator on THIS object (Kiwi root)
    [Tooltip("Pivot that should yaw with the model. Use Joint (preferred).")]
    public Transform model;                  // set this to the Joint object, not Kiwi
    ...
    void Awake()
    {
        ...
        if (model == null)
        {
            ...
            {
                // last resort - look for Kiwi model
                var k = transform.Find("Kiwi");
                if (k != null) model = k;
            }
        }
