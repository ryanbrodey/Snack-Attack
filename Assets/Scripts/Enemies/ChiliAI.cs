...
public class ChiliAI : MonoBehaviour
{
    [Header("References")]
    public Transform player;                 // auto-find by tag if empty
    public Animator animator;                // Animator on THIS object (Chili root)
    [Tooltip("Pivot that should yaw with the model. Use Joint (preferred).")]
    public Transform model;                  // set this to the Joint object, not Chilli
